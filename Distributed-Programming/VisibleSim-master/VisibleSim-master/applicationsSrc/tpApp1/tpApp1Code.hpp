...
class TpApp1Code : public BlinkyBlocksBlockCode {
  private:
    ...
    /**
      * @brief Turns this block into the (new) leader and starts a fresh
      *        gradient flood. Called either at startup (if the block is
      *        configured as leader) or when the block is tapped.
      *        Increments currentRound so that every other block, even
      *        ones that already settled on a smaller distance from the
      *        previous leader, accepts this new wave and recomputes
      *        its gradient value from the new source.
      */
    void startElection();
